    ...
    async def _process_memory_directory(
        ...
        try:
            wrote_semantics = await self._write_memory_directory_semantics(
                ...
                expires_at=(
                    min(expiry for _, expiry in ttl_snapshot.values()) if ttl_snapshot
        ...
        if msg.skip_vectorization:
            ...
            return
        if ttl_snapshot is None and not (
            wrote_semantics.overview_body_changed or wrote_semantics.abstract_body_changed
        ):
            logger.info(
