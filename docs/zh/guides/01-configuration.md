...
未配置 Rerank 时，候选基础分数来自向量检索；最终排序还受下方的分数传播与 hotness 配置影响。
...
`hotness_alpha=0.0` 只关闭热度加权，不能单独保证返回值等于原始向量相似度；还需要考虑 Rerank、分数传播和向量后端的计分方式。希望高频访问或最近更新的上下文获得排序提升时，可调高该值，并用代表性查询比较排序变化。
...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `default_enabled` | bool | 对既无显式 `auto_commit_policy`、也无部署级默认 policy 的新 session，是否默认开启 auto commit | `false` |
