...
先从用户已说明的目标和现有连接信息确认服务地址。目标不明确时，再询问要连接托管服务、远程自建还是本地自建；不要仅因默认端口可用就替用户选择本地服务。确定目标后，在已授权的范围内完成配置和连接检查。
...
1. 确认连接目标、配置名称与凭据类型。已有明确指示时直接沿用；替换无关配置、启动额外服务或导入数据超出本次授权时，再确认范围。
2. 查看当前 CLI 的相关帮助，设置显示语言。使用稳定的 `--name` 和非交互式配置命令，避免重试生成重复配置。
3. 通过 stdin 传入密钥；密钥已在环境变量中时可用 `--api-key-env` 或 `--root-api-key-env`。不要把密钥写入命令文本或日志，也不必让用户另开 shell 只为 export 密钥。
4. 使用 `-o json`，同时检查进程退出码和结果内容。完成后运行配置校验和健康检查；不要仅凭命令退出成功判断服务健康。
5. 必要信息仍缺失或密钥应由用户直接输入时，使用 `ov config` 交互向导。交付时说明生效的服务、配置名称及验证结果，不附带密钥。
...
添加资源会把数据写入 active OpenViking 服务端。如果你想做一个小演示，请选择你愿意存入服务端的资源。Agent 仅在用户已要求导入或演示时执行；单纯验证连接无需写入资源。
