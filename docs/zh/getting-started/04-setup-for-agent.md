...
- 先读取已有配置和用户已给出的信息，只询问缺失的模型、认证和数据目录要求；不要重复确认已明确的选择
...
必要信息齐全后即可写入或更新 `~/.openviking/ov.conf`，已有配置先保留副本。不要把下面的占位值直接作为可用配置。
...
`/health` 只确认进程可响应；缺配置的容器会返回 `503 pending_initialization`，不能只看 curl 是否收到了响应。启动后检查日志和 `/ready`，再完成一次小文件导入：
