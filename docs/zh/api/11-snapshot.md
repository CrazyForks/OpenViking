...
| file_paths | List[str] | 否 | null | 调用方确定为文件的 `viking://` URI 列表。快照内容与放在 `paths` 中相同，区别只在锁：这些条目取 Exact 锁而不是 Tree 锁，Exact 锁不会在目标缺失时创建目录，因此已删除的文件会保持缺失并被记录为删除。可与 `paths` 同时传入；USER/ADMIN 至少要传其中之一 |
