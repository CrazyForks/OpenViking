...
| tags | string[] | No | None | Retrieval tags in `k=v` format, written with generated vector records and usable as search filters |
| tag_mode | string | No | `replace` | Replace existing tags or merge by key with `append`; no separate `tags_result` is returned |
