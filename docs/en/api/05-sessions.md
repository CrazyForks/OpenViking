...
All fields have a minimum of `0` and are clamped into `[0, max]`. Unknown keys are rejected with `InvalidArgumentError`. Idle commits also require `memory.session_auto_commit.idle_enabled` on the server; setting `idle_timeout_seconds` alone does not start the scanner.
