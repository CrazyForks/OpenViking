...
Resource Compile defaults to `6` parallel source children and `10` parallel merge children, controlled by `CompileLimits.source_concurrency` and `CompileLimits.merge_concurrency`, excluding the parent. The runtime switches to merge concurrency after all source tasks finish and their results are collected. Running tasks, queued tasks, and uncollected results together are capped at twice the current phase limit (`12` and `20` by default); queued work starts automatically. Other Compile targets use `bot.agents.subagent_max_concurrency` (default `8`). Source dispatch follows the capacity returned by `wait_subagents`; Resource merge plans are scheduled by the runtime, and retries cover only pending inputs. The agent decides topics; the runtime verifies draft coverage, paths, source retention and OKF formatting.
