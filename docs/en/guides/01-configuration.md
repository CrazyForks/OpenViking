...
| Parameter | Type | Description | Default |
|-----------|------|-------------|---------|
| `default_enabled` | bool | Enables auto commit by default for new sessions with neither an explicit `auto_commit_policy` nor a deployment-default policy | `false` |
